class Day6 {
    ...
    private val durations = parseLine(input[0])
    private val distances = parseLine(input[1])

    private fun parseLine(line : String) : List<Long> {
        return Regex("\\d+").findAll(line).map(MatchResult::value).map(String::toLong).toList()
    }
